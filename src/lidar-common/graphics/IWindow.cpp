...
bool IWindow::s_keyboardState[SDL_NUM_SCANCODES] = {0};

bool IWindow::Init()
{
  // Create a window
  m_window = SDL_CreateWindow(m_windowName.data(), SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, m_windowWidth, m_windowHeight, SDL_WINDOW_VULKAN | SDL_WINDOW_ALLOW_HIGHDPI | m_extraFlags);
  if (!m_window)
  {
  ...
  SRenderer->CreateSwapchain(SCDesc, Diligent::NativeWindow(m_window, true), &m_swapchain);
  if (!m_swapchain)
  {
