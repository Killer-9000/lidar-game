...
namespace ifd {
	...
	bool FolderNode(const char* label, ImTextureID icon, bool& clicked)
	{
		...
		{
			ImRect rect(ImVec2(pos.x, pos.y + g.Style.FramePadding.y), ImVec2(pos.x + 0.750f, pos.y + g.Style.FramePadding.y + 0.866f));
			if (ImGui::ItemAdd(rect, 0))
				ImGui::RenderArrow(window->DrawList, rect.Min, ImGui::ColorConvertFloat4ToU32(ImGui::GetStyle().Colors[((hovered && is_mouse_x_over_arrow) || opened) ? ImGuiCol_Text : ImGuiCol_TextDisabled]), opened ? ImGuiDir_Down : ImGuiDir_Right);
		}
		{
			ImRect rect(ImVec2(icon_posX, pos.y), ImVec2(icon_posX + ICON_SIZE, pos.y + ICON_SIZE));
			if (ImGui::ItemAdd(rect, 0))
				window->DrawList->AddImage(icon, rect.Min, rect.Max);
		}
	...
	bool PathBox(const char* label, std::filesystem::path& path, char* pathBuffer, ImVec2 size_arg) {
		...
		if (!(*state & 0b001)) {
			...
			std::vector<std::string> btnList; btnList.reserve(20);
	...
	FileDialog::FileDialog() {
		...
		m_currentDirectory = (m_currentDirectory == "") ? std::filesystem::current_path() : m_currentDirectory;
		...
		quickAccess->Children.reserve(20);
		...
		// OneDrive
		m_treeCache.push_back(new FileTreeNode(userPath + L"OneDrive"));
		...
		for (int i = 0; i < 26; i++)
		{
			if (d & (1 << i))
			{
				auto driveLetter = fmt::sprintf("%c:\\", 'A' + i);
				CHAR driveName[MAX_PATH];
				GetVolumeInformationA(driveLetter.c_str(), driveName, MAX_PATH, NULL, NULL, NULL, NULL, 0);
				if (strcmp(driveName, "") == 0)
					strcpy(driveName, "Local Disk");
				thisPC->Children.push_back(new FileTreeNode(fmt::sprintf("%c:/", 'A' + i)));
				thisPC->Children.back()->PathAlias = fmt::sprintf("%s (%c:)", driveName, 'A' + i);
			}
		}
	...
	bool FileDialog::Open(const std::string& key, const std::string& title, const std::string& filter, bool isMultiselect, bool allowFolders, const std::string& startingDir)
	{
		if (!m_currentKey.empty())
			return false;
...
#ifdef _WIN32
		if (m_icons.count(pathU8) > 0) [[likely]]
	...
	void FileDialog::m_clearIcons()
	{
		...
		for (auto& [path, icon] : m_icons) {
			unsigned int ptr = (unsigned int)((uintptr_t)icon);
			...
			for (auto& node : m_treeCache) {
				if (node->Path == p)
				{
					for (auto& c : node->Children)
					{
						...
						m_content.back().PathAlias = c->PathAlias;
					}
				}
			}
		}
	...
	void FileDialog::m_renderTree(FileTreeNode* node)
	{
		...
		if (node->PathAlias.size())
			displayName = node->PathAlias;
		else
		{
			displayName = node->Path.stem().string();
		...
		if (m_zoom == 1.0f) {
			if (ImGui::BeginTable("##contentTable", 3, /*ImGuiTableFlags_Resizable |*/ ImGuiTableFlags_Sortable, ImVec2(0, -FLT_MIN))) {
				...
				for (auto& entry : m_content) {
					std::string filename;
					if (entry.PathAlias.size())
						filename = entry.PathAlias;
					else
					{
					...
					if (ImGui::Selectable(filename.c_str(), isSelected, ImGuiSelectableFlags_SpanAllColumns | ImGuiSelectableFlags_AllowDoubleClick)) {
						...
						if (ImGui::IsMouseDoubleClicked(0)) {
							...
							if (((isDir && m_type == IFD_DIALOG_DIRECTORY) || !isDir) || m_allowFolders)
					...
					if (tm != nullptr)
						ImGui::Text("%d/%d/%d %02d:%02d", tm->tm_mday, tm->tm_mon + 1, 1900 + tm->tm_year, tm->tm_hour, tm->tm_min);
					else ImGui::Text("---");
					...
					{
						if (entry.Size > 1024ll * 1024 * 1024 * 1024)
							ImGui::Text("%.3f TiB", entry.Size / (1024.0f * 1024.0f * 1024.0f * 1024.0f));
						else if (entry.Size > 1024 * 1024 * 1024)
							ImGui::Text("%.3f GiB", entry.Size / (1024.0f * 1024.0f * 1024.0f));
						else if (entry.Size > 1024 * 1024)
							ImGui::Text("%.3f MiB", entry.Size / (1024.0f * 1024.0f));
						else if (entry.Size > 1024)
							ImGui::Text("%.3f KiB", entry.Size / 1024.0f);
						else
							ImGui::Text("%i Bytes", entry.Size);
					}
				}
	...
	void FileDialog::m_renderPopups()
	{
		...
		if (ImGui::BeginPopupContextItem("##dir_context")) {
			...
			if (m_selectedFileItem != -1 && ImGui::Selectable("Open in File Explorer"))
			{
				std::string filepath;
				if (m_content[m_selectedFileItem].IsDirectory)
					filepath = m_content[m_selectedFileItem].Path.string();
				else
					filepath = m_content[m_selectedFileItem].Path.parent_path().string();
	...
	void FileDialog::m_renderFileDialog()
	{
		...
		if (ImGui::Button(m_type == IFD_DIALOG_SAVE ? "Save" : "Open", ImVec2(ok_cancel_width / 2 - ImGui::GetStyle().ItemSpacing.x, 0.0f)))
		{
			if (m_allowFolders)
			{
				...
				{
					bool success = false;
					success = m_finalize(m_selections.back().string());
#ifdef _WIN32
					if (!success)
						MessageBeep(MB_ICONERROR);
#else
				...
				{;
					bool success = false;
					if (!m_currentDirectory.empty() || m_type == IFD_DIALOG_DIRECTORY)
						success = m_finalize(m_currentDirectory.string());
#ifdef _WIN32
					if (!success)
						MessageBeep(MB_ICONERROR);
#else
			...
			else if (m_selections.size())
			{
				if (std::filesystem::is_directory(m_selections.back()))
					m_setDirectory(m_selections.back());
				else
				{
					bool success = false;
						success = m_finalize(m_selections.back().string());
