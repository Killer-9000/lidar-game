...
//#include <util/RemoteSessionChecker.h>
...
protected:
	...
	std::chrono::nanoseconds m_frameStart = 0ms;
	std::chrono::nanoseconds m_frameEnd = 1ms;
	float m_frameDelta = 16.0f;
	...
	const bool* GetKeyboardState()
	{
		//if (!IsCurrentSessionRemoteable())
			//return (bool*)SDL_GetKeyboardState(nullptr);
		//else
			return s_keyboardState;
	}

	static bool s_keyboardState[SDL_NUM_SCANCODES];
};
