...
public:
	...
	IArchive::Type GetType() const
	{
		IArchive::Type type = IArchive::Type::None;
		for (const auto& archive : m_archives)
			type = (archive->GetArchiveType() != IArchive::Type::Folder) ? archive->GetArchiveType() : type;
		return type;
	}
	...
	const ArchiveVector::const_reference GetArchive(std::string_view name, IArchive::Type type) const
	{
		auto ref = std::ranges::find_if(m_archives, [&](const std::shared_ptr<IArchive>& itr) {
	...
	bool OpenFile(std::string_view filename, uint8_t** outData, uint32_t* outSize)
	{
		...
		for (ArchiveVector::reverse_iterator itr = m_archives.rbegin(); itr != m_archives.rend(); itr++)
		{
			...
			{
				if (itr->get()->OpenFile(filename, outData, outSize))
					return true;
			}
		}
		...
		if (std::filesystem::exists(filename))
		{
			...
			if (!file.IsOpen())
				return false;
			*outData = (uint8_t*)malloc(file.GetSize());
			*outSize = (uint32_t)file.GetSize();
			memcpy(*outData, file.GetData(), file.GetSize());
		}
